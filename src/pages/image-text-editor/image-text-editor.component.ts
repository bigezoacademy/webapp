...
@Component({
  ...
  styleUrls: ['./image-text-editor.component.css']
})
export class ImageTextEditorComponent {
  textInputs: { text: string, color: string, size: string, font: string, fontWeight: string, fontStyle: string }[] = [];
  ...
  addTextInput() {
    this.textInputs.push({
      text: '', color: '#FFFF00', size: '16px', font: 'Roboto', fontWeight: 'normal', fontStyle: 'normal'
    }); // Default font, color, size, weight, and style
  }
  ...
  createImage() {
    ...
    img.onload = () => {
      ...
      this.textInputs.forEach(input => {
        ctx.font = `${input.fontWeight} ${input.fontStyle} ${input.size} ${input.font}, serif`;  // Include the selected font, weight, and style
